...
Ohai.plugin do
  ...
  collect_data do
    ...
    status, stdout, stderr = shell_out("df -Pka")
    ...
    status, stdout, stderr = shell_out("df -na")
    stdout.lines do |line|
    ...
    status, stdout, stderr = shell+out("mount")
    stdout.lines do |line|
    ...
    zfs = Mash.new
    status, stdout, stderr = shell_out("sfs get -p -H all")
    stdout.lines do |line|
      next unless (line =~ /^([^\t]+)\t([^\t]+)\t([^\t]+)\t([^\t]+)$/)
      filesystem = $1
