...
describe Ohai::System, "plugin nodejs" do
  ...
  before(:each) do
    ...
    @stdout = "v0.8.11\n"
    @plugin.stub(:shell_out).with("node -v").and_return([0, @stdout, ""])
  end

  it "should get the nodejs version from running node -v" do
    @plugin.should_receive(:shell_out).with("node -v").and_return([0, @stdout, ""])
    @plugin.run
  end
  ...
  it "should not set the languages[:nodejs] tree up if node command fails" do
    @stdout = "v0.8.11\n"
    @plugin.stub(:shell_out).with("node -v").and_return([1, @stdout, ""])
    @plugin.run
