...
describe Ohai::System, "plugin kernel" do
  before(:each) do
    ...
    @plugin.stub(:shell_out).with("uname -s").and_return([0, "Darwin\n", ""])
    @plugin.stub(:shell_out).with("uname -r").and_return([0, "9.5.0\n", ""])
    @plugin.stub(:shell_out).with("uname -v").and_return([0, "Darwin Kernel Version 9.5.0: Wed Sep  3 11:29:43 PDT 2008; root:xnu-1228.7.58~1\/RELEASE_I386\n", ""])
    @plugin.stub(:shell_out).with("uname -m").and_return([0, "i386\n", ""])
    @plugin.stub(:shell_out).with("uname -o").and_return([0, "Linux\n", ""])
  end
