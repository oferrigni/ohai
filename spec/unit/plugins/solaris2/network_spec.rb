...
ROUTE_GET
    ...
    @plugin.stub(:shell_out).with("ifconfig -a").and_return([0, @solaris_route_get, ""])
    @plugin.stub(:shell_out).with("arp -an").and_return([0, @solaris_arp_rn, ""])
    @plugin.stub(:shell_out).with("route -n get default").and_return([0, @soalris_route_get, ""])
  end

  describe "gathering IP layer address info" do
    before do
      @stdout = double("Pipe, stdout, cmd=`route get default`", :read => @solaris_route_get)
      @plugin.stub(:shell_out).with("route -n get default").and_return([0, @solaris_route_get, ""])
      @plugin.stub(:shell_out).with("ifconfig -a").and_return([0, @solaris_ifconfig, ""])
      @plugin.run
    end
