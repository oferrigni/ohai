...
describe Ohai::System, "plugin java (Java5 Client VM)" do
  before(:each) do
    ...
    @plugin.stub(:shell_out).with("java -version").and_return([0, "", @stderr])
  end

  it "should run java -version" do
    @plugin.should_receive(:shell_out).with("java -version").and_return([0, "", @stderr])
    @plugin.run
  end
  ...
  it "should not set the languages[:java] tree up if java command fails" do
    ...
    @plugin.stub(:shell_out).with("java -version").and_return([1, "", @stderr])
    @plugin.run
