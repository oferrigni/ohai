...
describe Ohai::System, "plugin rackspace" do
  ...
  @plugin[:network][:interfaces][:eth1] = {:addresses => {
    ...
    # Because, magic.
    @plugin.stub(:shell_out).and_return(false)
  end
...
region = "dfw"
OUT
      @plugin.stub(:shell_out).with("xenstore-ls vm-data/provider_data").and_return([ 0, provider_data, ""])
      @plugin.run
  ...
  describe "xenstore provider returns rackspace" do
    ...
    before(:each) do
      stdout = "Rackspace\n"
      @plugin.stub(:shell_out).with("xenstore-read vm-data/provider_data/provider").and_return([ 0, stdout, "" ])
    end
  end
