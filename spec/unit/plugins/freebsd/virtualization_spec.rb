...
describe Ohai::System, "FreeBSD virtualization plugin" do
  before(:each) do
    ...
    @plugin[:os] = "freebsd"
    @plugin.stub(:shell_out).with("sysctl -n security.jail.jailed").and_return([0, "0", ""])
    @plugin.stub(:shell_out).with("#{ Ohai.abs_path( "/sbin/kldstat" )}").and_return([0, "", ""])
    @plugin.stub(:shell_out).with("jls -n").and_return([0, "",""])
    @plugin.stub(:shell_out).with("sysctl -n hw.model").and_return([0, "", ""])
  end

  context "jails" do
    it "detects we are in a jail" do
      @plugin.stub(:shell_out).with("sysctl -n security.jail.jailed").and_return([0, "1", ""])
      @plugin.run
    ...
    @jails = "JID  IP Address      Hostname                      Path\n     3  192.168.3.17    ns.example.org                /home/j/ns\n     2  192.168.3.18    mail.example.org              /home/j/mail\n     1  62.123.43.14    www.example.org               /home/j/www"
      @plugin.stub(:shell_out).with("jls -n").and_return([0, @jails, ""])
      @plugin.run
      @plugin[:virtualization][:system].should == "jail"
...
OUT
      @plugin.stub(:shell_out).with("#{ Ohai.abs_path( "/sbin/kldstat" )}").and_return([0, @vbox_guest, ""])
    end
...
OUT
      @plugin.stub(:shell_out).with("/sbin/kldstat").and_return([0, @stdout, ""])
    end
  ...
  context "when on a QEMU guest" do
    it "detects we are a guest" do
      ...
      @plugin.stub(:shell_out).with("sysctl -n hw.model").and_return([0, @qemu_guest, ""])
      @plugin.run
