...
Ohai.plugin do
  ...
  collect_data do
    ...
    #gcc
    status, stdout, stderr = shell_out("gcc -v")
    if status == 0
    ...
    ["/lib/libc.so.6", "/lib64/libc.so.6"].each do |glibc|
      status, stdout, stderr = shell_out(glibc)
      if status == 0
    ...
    #ms cl
    status, stdout, stderr = shell_out("cl /?")
    if status == 0
    ...
    #ms vs
    status, stdout, stderr = shell_out("devenv.com /?")
    if status == 0
      lines = stdout.split($/)
    ...
    #ibm xlc
    status, stdout, stderr = shell_out("xlc -qversion")
    ...
    #sun pro
    status, stdout, stderr = shell_out("cc -V -flags")
    if status == 0
      output = stderr.split
    ...
    #hpux cc
    status, stdout, stderr = shell_out("what /opt/ansic/bin/cc")
    if status == 0
