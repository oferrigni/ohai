...
describe Ohai::System, "plugin perl" do
  before(:each) do
    ...
    @plugin.stub(:shell_out).with("perl -V:version -V:archname").and_return([0, @stdout, ""])
  end

  it "should run perl -V:version -V:archname" do
    @plugin.should_receive(:shell_out).with("perl -V:version -V:archname").and_return([0, @stdout, ""])
    @plugin.run
  end
  ...
  it "should not set languages[:perl] if perl command fails" do
    @status = 1
    @plugin.stub(:shell_out).with("perl -V:version -V:archname").and_return([1, @stdout, ""])
    @plugin.run
