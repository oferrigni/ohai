...
Ohai.plugin do
  ...
  collect_data do
    ...
    status, stdout, stderr = shell_out("sysctl -n security.jail.jailed")
    if stdout.split($/)[0].to_i == 1
      virtualization[:system] = "jail"
      virtualization[:role] = "guest"
    end

    # detect from modules
    status, stdout, stderr = shell_out("#{ Ohai.abs_path( "/sbin/kldstat" )}")
    ...
    status, stdout, stderr = shell_out("jls -n")
    ...
    status, stdout, stderr = shell_out("sysctl -n hw.model")
    if stdout.split($/)[0] =~ /QEMU Virtual CPU/
    ...
    if File.exists?("/usr/local/sbin/dmidecode")
      status, stdout, stderr = shell_out("dmidecode")
