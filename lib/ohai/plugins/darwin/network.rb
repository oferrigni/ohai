...
Ohai.plugin do
  ...
  collect_data do
    ...
    status, stdout, stderr = shell_out("route -n get default")
    ...
    iface = Mash.new
    status, stdout, stderr = shell_out("ifconfig -a")
    cint = nil
    stdout.lines do |line|
    ...
    status, stdout, stderr = shell_out("arp -an")
    stdout.lines do |line|
    ...
    settings = Mash.new
    status, stdout, stderr = shell_out("sysctl net")
    stdout.lines do |line|
      if line =~ /^([a-zA-Z0-9\.\_]+)\: (.*)/
    ...
    status, stdout, stderr = shell_out("netstat -i -d -l -b -n")
    stdout.lines do |line|
