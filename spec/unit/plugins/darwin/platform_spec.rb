...
describe Ohai::System, "Darwin plugin platform" do
  before(:each) do
    ...
    @plugin.stub(:shell_out).with("/usr/bin/sw_vers").and_return([0, @stdout, ""])
  end
 
  it "should run sw_vers" do
    @plugin.should_receive(:shell_out).with("/usr/bin/sw_vers").and_return([0, @stdout, ""])
    @plugin.run
  end

  # TODO. Not sure how to test this against Mixlib::ShellOut.
  #   ... or if we need to.
