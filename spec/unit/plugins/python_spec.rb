...
describe Ohai::System, "plugin python" do
  ...
  before(:each) do
    ...
    @plugin.stub(:shell_out).with("python -c \"import sys; print sys.version\"").and_return([0, @stdout, ""])
  end
  
  it "should get the python version from printing sys.version and sys.platform" do
    @plugin.should_receive(:shell_out).with("python -c \"import sys; print sys.version\"").and_return([0, @stdout, ""])
    @plugin.run
  end
  ...
  it "should not set the languages[:python] tree up if python command fails" do
    @stdout = "2.5.2 (r252:60911, Jan  4 2009, 17:40:26)\n[GCC 4.3.2]\n"
    @plugin.stub(:shell_out).with("python -c \"import sys; print sys.version\"").and_return([1, @stdout, ""])
    @plugin.run
