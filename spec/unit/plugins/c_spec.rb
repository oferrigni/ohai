...
describe Ohai::System, "plugin c" do
  ...
  before(:each) do
    ...
    #gcc
    @plugin.stub(:shell_out).with("gcc -v").and_return([0, "", C_GCC])
    #glibc
    @plugin.stub(:shell_out).with("/lib/libc.so.6").and_return([0, C_GLIBC_2_3_4, ""])
    #ms cl
    @plugin.stub(:shell_out).with("cl /\?").and_return([0, "", C_CL])
    #ms vs
    @plugin.stub(:shell_out).with("devenv.com /\?").and_return([0, C_VS, ""])
    #ibm xlc
    @plugin.stub(:shell_out).with("xlc -qversion").and_return([0, C_XLC, ""])
    #sun pro
    @plugin.stub(:shell_out).with("cc -V -flags").and_return([0, "", C_SUN])
    #hpux cc
    @plugin.stub(:shell_out).with("what /opt/ansic/bin/cc").and_return([0, C_HPUX, ""])
  end

  #gcc
  it "should get the gcc version from running gcc -v" do
    @plugin.should_receive(:shell_out).with("gcc -v").and_return([0, "", C_GCC])
    @plugin.run
  end
  ...
  it "should not set the languages[:c][:gcc] tree up if gcc command fails" do
    @plugin.stub(:shell_out).with("gcc -v").and_return([1, "", ""])
    @plugin.run
  ...
  it "should get the glibc x.x.x version from running /lib/libc.so.6" do
    @plugin.should_receive(:shell_out).with("/lib/libc.so.6").and_return([0, C_GLIBC_2_3_4, ""])
    @plugin.run
  end
  ...
  it "should not set the languages[:c][:glibc] tree up if glibc command fails" do
    @plugin.stub(:shell_out).with("/lib/libc.so.6").and_return([1, "", ""])
    @plugin.stub(:shell_out).with("/lib64/libc.so.6").and_return([1, "", ""])
    @plugin.run
  ...
  it "should get the glibc x.x version from running /lib/libc.so.6" do
    @plugin.stub(:shell_out).with("/lib/libc.so.6").and_return([0, C_GLIBC_2_5, ""])
    @plugin.should_receive(:shell_out).with("/lib/libc.so.6").and_return([0, C_GLIBC_2_5, ""])
    @plugin.run
  ...
  it "should get the cl version from running cl /?" do
    @plugin.should_receive(:shell_out).with("cl /\?").and_return([0, "", C_CL])
    @plugin.run
  end
  ...
  it "should not set the languages[:c][:cl] tree up if cl command fails" do
    @plugin.stub(:shell_out).with("cl /\?").and_return([1, "", ""])
    @plugin.run
  ...
  it "should get the vs version from running devenv.com /?" do
    @plugin.should_receive(:shell_out).with("devenv.com /\?").and_return([0, C_VS, ""])
    @plugin.run
  end
  ...
  it "should not set the languages[:c][:vs] tree up if devenv command fails" do
    @plugin.stub(:shell_out).with("devenv.com /\?").and_return([1, "", ""])
    @plugin.run
  ...
  it "should get the xlc version from running xlc -qversion" do
    @plugin.should_receive(:shell_out).with("xlc -qversion").and_return([0, C_XLC, ""])
    @plugin.run
  end
  ...
  it "should not set the languages[:c][:xlc] tree up if xlc command fails" do
    @plugin.stub(:shell_out).with("xlc -qversion").and_return([1, "", ""])
    @plugin.run
  ...
  it "should set the languages[:c][:xlc] tree up if xlc exit status is 249" do
    @plugin.stub(:shell_out).with("xlc -qversion").and_return([63744, "", ""])
    @plugin.run
    @plugin[:languages][:c].should_not have_key(:xlc) if @plugin[:languages][:c]
  end
  ...
  it "should get the cc version from running cc -V -flags" do
    @plugin.should_receive(:shell_out).with("cc -V -flags").and_return([0, "", C_SUN])
    @plugin.run
  end
  ...
  it "should not set the languages[:c][:sunpro] tree up if cc command fails" do
    @plugin.stub(:shell_out).with("cc -V -flags").and_return([1, "", ""])
    @plugin.run
  ...
  it "should not set the languages[:c][:sunpro] tree if the corresponding cc command fails on linux" do
    ...
    @plugin.stub(:shell_out).with("cc -V -flags").and_return([0, "", fedora_error_message])
    @plugin.run
    @plugin[:languages][:c].should_not have_key(:sunpro) if @plugin[:languages][:c]
  end
  ...
  it "should not set the languages[:c][:sunpro] tree if the corresponding cc command fails on hpux" do
    ...
    @plugin.stub(:shell_out).with("cc -V -flags").and_return([0, "", hpux_error_message])
    @plugin.run
    @plugin[:languages][:c].should_not have_key(:sunpro) if @plugin[:languages][:c]
  end
  ...
  it "should get the cc version from running what cc" do
    @plugin.should_receive(:shell_out).with("what /opt/ansic/bin/cc").and_return([0, C_HPUX, ""])
    @plugin.run
  end
  ...
  it "should not set the languages[:c][:hpcc] tree up if cc command fails" do
    @plugin.stub(:shell_out).with("what /opt/ansic/bin/cc").and_return([1, "", ""])
    @plugin.run
