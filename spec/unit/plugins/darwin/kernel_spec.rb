...
describe Ohai::System, "Darwin kernel plugin" do
  before(:each) do
    ...
    @plugin.should_receive(:shell_out).with("kextstat -k -l").and_return([0, "", ""])
  end

  it "should not set kernel_machine to x86_64" do
    @plugin.stub(:shell_out).with("sysctl -n hw.optional.x86_64").and_return([0, "0", ""])
    @plugin.run
    @plugin[:kernel][:machine].should_not == 'x86_64'
  end

  it "should set kernel_machine to x86_64" do
    @plugin.stub(:shell_out).with("sysctl -n hw.optional.x86_64").and_return([0, "1", ""])
    @plugin.run
