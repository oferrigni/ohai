...
net.smb.fs.tcprcvbuf: 261120
    ...
    @plugin.stub(:shell_out).with("route -n get default").and_return([0, @darwin_route, ""])
    ...
    %w{ darwin/hostname hostname network }.each do |plgn|
      p = get_plugin(plgn)
      p.stub(:shell_out).with("hostname -s").and_return([0, "katie", ""])
      p.stub(:shell_out).with("hostname").and_return([0, "katie.bethell", ""])
      p.run
    end
  end
  ...
  describe "gathering IP layer address info" do
    before do
      @plugin.stub(:shell_out).with("arp -an").and_return([0, @darwin_arp, ""])
      @plugin.stub(:shell_out).with("ifconfig -a").and_return([0, @darwin_ifconfig, ""])
      @plugin.stub(:shell_out).with("netstat -i -d -l -b -n").and_return([0, @darwin_netstat, ""])
      @plugin.stub(:shell_out).with("sysctl net").and_return([0, @darwin_sysctl, ""])
      @plugin.run
    end
